...
local function Build()
    local tab = SlerneNotes.settingsTab
    ...
    local verRefresh = CreateFrame("Button", nil, verPanel, "UIPanelButtonTemplate")
    verRefresh:SetSize(90, 24)
    verRefresh:SetPoint("TOPRIGHT", -14, -12)
    verRefresh:SetText("Refresh")
    verRefresh:SetScript("OnClick", function()
        if SlerneNotes.RequestViewerVersions then SlerneNotes.RequestViewerVersions(0.1) end
        SlerneNotes.RefreshViewerVersions()
    end)
    SlerneNotes.Skin.Button(verRefresh)
...
function SlerneNotes.RefreshViewerVersions()
    if not verPanel or not (SlerneNotes.settingsTab and SlerneNotes.settingsTab:IsShown()) then return end
...
function SlerneNotes.RefreshSettingsUI()
    Build()
